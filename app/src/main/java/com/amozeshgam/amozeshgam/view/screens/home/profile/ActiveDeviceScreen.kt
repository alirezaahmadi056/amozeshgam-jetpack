...
fun ViewActiveDevice(
    ...
    val activeDevicesData = remember {
        mutableStateOf<ApiResponseGetDevices?>(null)
    }
    ...
    UiHandler.ContentWithScaffold(
        ...
        UiHandler.ContentWithLoading(
            ...
            ifForShowContent = activeDevicesData.value != null,
            worker = {
                activeDevicesData.value = viewModel.getActiveDevices().await()
            ...
            LazyColumn(
                ...
                items(activeDevicesData.value!!.devices.size) { index ->
                    ActiveDevicesItem(
                        deviceName = activeDevicesData.value!!.devices[index].deviceName,
                        deviceModel = activeDevicesData.value!!.devices[index].deviceId,
                        deviceIP = ""
                    ) {
                        viewModel.deActiveDevice(deviceId = activeDevicesData.value!!.devices[index].deviceId)
                    }
                    if (index != activeDevicesData.value!!.devices.size) {
                        HorizontalDivider(
                            modifier = Modifier
                                .fillMaxSize()
